...
export default function AddSubSkill(props) {
  ...
  // Get Sub Skill
  const getSubSkill = async (id) => {
    setopensubcat(false);
    setopenUpdatesubcatmodal(true);
    const res = await axios.get(`https://football-backend-updated.herokuapp.com/skill/getAllsubskillsofskill/${id}`);
    setsubskillname(res.data.data.subskillname);
    setsubcat(res.data.data._id);
  ...
  const deleteSkill = async () => {
    setRefresh(true);
    const res = await axios.delete(`https://football-backend-updated.herokuapp.com/skill/DeleteSkill/${localData.data._id}`);
    setRefresh(false);
    props.refreshData();
    console.log(res.data);
  };
  ...
  const updateSkill = async (e) => {
    e.preventDefault();
    setRefresh(true);
    const res = await axios.put(`https://football-backend-updated.herokuapp.com/skill/UpdateSkill/${localData.data._id}`, {
      skillname: skill,
      skillicon: url,
    });
    setRefresh(false);
  ...
  return (
    <>
      <div>
        {opensubcat === false ? (
          <div style={{ position: "relative" }} onMouseEnter={() => setButton(true)}
          onMouseLeave={() => setButton(false)}>
            ...
            >
              <div className="flex justify-center pt-5 ">
                <img src={localData.data.skillicon} className="w-[64px] h-[64px]" />
              </div>
            ...
            <>
              <div className="my-3 p-3 max-w-sm max-h-[180px] rounded-lg bg-[#212121]">
                <div className="flex items-center ">
                  ...
                  <button
                    className="ml-auto"
                    onClick={() => deleteSubSkill(val.refOfSkill, val._id)}
                  >
                    <svg
                      className="ml-auto"
                      width="12"
