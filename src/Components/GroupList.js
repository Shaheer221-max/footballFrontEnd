...
export default function GroupList() {
  ...
  const getData = async () => {
    let res = await axios.get("https://football-backend-updated.herokuapp.com/users/me", {
      headers: {
  ...
  const data = async() => {
    let res = await axios.get("https://football-backend-updated.herokuapp.com/group/GetAllGroups/")
    .then((res) => {
    ...
    <>
      <div className=" font-lexend flex items-center justify-between mb-7  mt-10">
        ...
        <NavLink to={"/selectgroup"} >
          <p className="text-sm text-gray-500 ml-auto">All</p></NavLink>
      </div>
      {groups === false ? (<></>) : (<>
        {groups?.map((val, ind) => (
        <NavLink to={"/selectgroup"}
