...
export default function GroupMembers(props) {
  ...
  const removeMember = async (id) => {
    await axios
      .post(
        `https://football-backend-updated.herokuapp.com/group/RemoveMember/${id}&${props.data.val._id}`
      )
      .then((res) => {
        console.log(res.data);
      })
  ...
  const addMember = async (id) => {
    await axios
      .post(
        `https://football-backend-updated.herokuapp.com/group/AddMember/${id}&${props.data.val._id}`
      )
      .then((res) => {
        console.log(res.data);
      })
            ...
            <div className="w-full min-w-sm bg-transparent   ">
              <div className="px-4 pt-4">
                ...
                <div>
                  ...
                  {props.data.val.Members ? (
                    <>
                      {props.data.val.Members.map((val, ind) => {
                        return (
                          <div className="flex items-center font-lexend  mb-6">
                            {val.image ? (
                              <>
